...
uint8 permanentList[SIZE][6]=	{{0X39,0X00,0X13,0X01,0X00,0X06},	
...
CY_ISR (press) {  
  if (state == DISCONNECTED) {
    ...
    /* sprintf(buff,"INSIDE SECOND BUTTON PRESS\n\r");
     UART_PutString(buff);*/
     CyBle_GapcStopScan();
    SW_ClearInterrupt();
  }
...
CYBLE_GAPC_ADV_REPORT_T advreport = *(CYBLE_GAPC_ADV_REPORT_T *)eventParam;
  ...
  switch(event) {
    ...
    /*CyBle_GappStartAdvertisement || CyBle_GappStopAdvertisement() */
    case CYBLE_EVT_GAPP_ADVERTISEMENT_START_STOP:
       if (state == DISCONNECTED){  //this will be when advertising is started
          state = ADVERTISING;
        /*  sprintf(buff,"ADVERTISING\n\r");
          UART_PutString(buff);*/
    ...
    case CYBLE_EVT_GAPC_SCAN_PROGRESS_RESULT:
      if(advreport.eventType == CYBLE_GAPC_CONN_UNDIRECTED_ADV){
     	/*  sprintf(buff,"INSIDE SCAN PROGRESS RESULT\r\n");
     	  UART_PutString(buff);*/
  			uint8 val;
    ...
    if (state == CONNECTED && !attendanceMarked) { //this will be when advetising is stopped
      state = DISCONNECTED;
  ...
  /*    sprintf(buff,"State     :%u \n\r",state);
    ...
    /*    sprintf(buff,"WAITING FOR TEACHER TO END LECTURE\r\n");
      ...
      else if(state == DISCONNECTED){//this will be a jump from startADV when teacher pressed button first time
        state = SCANNING;
        myreg_Write(0x5);
      }
      break;
...
void IasEventHandler(uint32 event, void *eventParam) {
  ...
  //  
    ...
    case MILD_ALERT:
      // CyBle_GappStopAdvertisement();//possible error
      //not sure where the code will jump, to evtdisconnect or to evtstopadv
...
void HandleScanDevices(CYBLE_GAPC_ADV_REPORT_T* advReport)
{
	//sprintf(buff,"Inside HandleScanDevices \n\r");
//	UART_PutString(buff);
...
{
  for(i = 0; i < SIZE; i++) {                    //checking if class member or not
    ...
    //  UART_PutString(buff);
        ...
        if(member){          
       //   sprintf(buff,"here1\r\n");//check
       //   UART_PutString(buff);  // checks if already marked present
        ...
        //  UART_PutString(buff);  
          ...
          present_devices[addedDevices].data[4] = a[4];
            ...
            UART_PutString(buff);*/
						...
						sprintf(buff,"a after reset ya kuch: %2.2x%2.2x%2.2x%2.2x%2.2x%2.2x\r\n",a[5u],a[4u],a[3u],a[2u],a[1u],a[0u]);
            UART_PutString(buff);*/
						addedDevices++;
        }
    }
